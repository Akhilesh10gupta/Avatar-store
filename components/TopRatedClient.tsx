...
import { Trophy, Search, X, SearchX } from 'lucide-react';
...
export default function TopRatedClient({ initialGames }: TopRatedClientProps) {
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-in fade-in duration-500">
                        {filteredGames.map((game) => (
                            <GameCard
                                key={game.id}
                                game={game}
                                rank={initialGames.findIndex(g => g.id === game.id) + 1}
                            />
                        ))}
                    </div>
                ) : (
