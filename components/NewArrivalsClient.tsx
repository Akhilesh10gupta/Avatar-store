...
export default function NewArrivalsClient({ initialGames }: NewArrivalsClientProps) {
    const [searchQuery, setSearchQuery] = useState('');
    const [selectedCategory, setSelectedCategory] = useState('All');

    const categories = useMemo(() => {
        const cats = new Set(initialGames.map(g => g.genre).filter(Boolean));
    ...
    }, [initialGames, searchQuery, selectedCategory]);
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold md:text-4xl flex items-center gap-3">
                            <Sparkles className="text-yellow-400 w-6 h-6 md:w-8 md:h-8 fill-yellow-400" />
                            New Arrivals
                        </h1>
                        <p className="text-muted-foreground mt-2 max-w-2xl">
                            Be the first to play the latest additions to our collection.
